...
//
// Purpose: construct the exhaustive 245x245 matrices of all identifiable
...
//
// databases used: - "$temp/data_gravity_update.dta"
//                 - "$temp\data_tot`asset'_update.dta" (asset in "eq" "debt")
...
//                 - "$temp\data_ewn_update.dta"
//                 - "$temp\data_foreignexchange_update.dta"
//                 - "$temp\KY_banks.dta"
//                 - "$raw\dta\KY_liab_nfc.dta"
//                 - "$temp\BIS_total_debt_IO.dta"
//
// outputs:        - "$temp\data_full_matrices.dta"
//                 - "$temp\gap_source_update.dta"
...
// Definition of OFC for the regressions: the 42 countries reported in Table 2 of IMF (2000), except Switzerland
gen ofc_source=0
replace ofc_source=1 if sifc_source == 1

// Add Anguilla, Liechtenstein, BVI, Monaco, Cook Islands, Niue, Ireland, Luxembourg, Hong Kong, Singapore, Cyprus to OFC list
replace ofc_source = 1 if source == 312 | source == 147 | source == 371 | source == 183 | source == 815 | source == 851 | source == 178 | source == 137 | source == 532 | source == 576 | source == 423
...
replace ofc_source = 0 if source == 585 | source == 321 | source == 328 | source == 668 | source == 135 | source==298 | source == 351 | source == 565
...
// and same controls (except for capital controls; we could not find the data) */
...
/* Results: We get a somewhat smaller R2 than they do (0.76 instead of 0.79) and gap_long has the wrong sign.
So in what follows we drop it */
...
// some predicted values are very very slightly negative, we replace them by zero
...
// First, we compute source-level aggregates, because these are bigger than total claims + unallocated claims 
// --> So we want to put the residual in unallocated, and allocate the residual too


// a) we compute the raw sums
sort source year
by source year: egen toteqasset = total(eqasset)
by source year: egen totdebtasset = total(debtasset)

// b) we compare the raw sums with aggregates from CPIS data
merge m:1 source year using "$temp\data_toteq_update.dta" 
drop _merge cname
merge m:1 source year using "$temp\data_totdebt_update.dta"
drop _merge cname
...
// c) CONCLUSION = In the IMF CPIS raw datafiles (March 2024), the sum for each source country of its foreign claims is not always equal to the reported total, i.e. some unallocated claims are missing from the line "Not specified (inlcuding confidential)")
// we add them here >> this augments the line "not specified" 983) */
...
// a) we assume that all unallocated + confidential claims are vis-a-vis countries for which A_{ij} is either missing or zero or Andorra, or Liechtenstein or Monaco (in most cases A_{ij} is zero for these countries but sometimes not, which introduces spurrious variation in the allocation of the unallocated claims). we also disregard San Marino, a clear outlier.
// (NB: we have replaced all missing values by zeros at the beginning of this section) */ 
...
replace confidentialeq = 1 if (eqasset == 0 | host == 171 | host == 147 | host == 183) & toteqasset !=. & toteqasset != 0 & host != 135 & ignore_jur != 1 // 171 "Andorra", 147 "Liechtenstein", 183 "Monaco", 135 "San Marino"

gen confidentialdebt = 0
replace confidentialdebt = 1 if (debtasset == 0 | host == 171 | host == 147 | host == 183) & totdebtasset != . & totdebtasset != 0 & host != 135 & ignore_jur != 1 
...
// c) Merge these columns with my dataset */
sort year source
...
gen unalloceq = 0
replace unalloceq = shareeqpconf * totunalloceq if source != 93
gen unallocdebt = 0
replace unallocdebt = sharedebtpconf * totunallocdebt if source != 93
sort source year

// e) we create an augmented asset line, equal to the raw asset line + allocated confidential+unalloc
// we set the unallocated and confidential lines to zero

gen augmeqasset = eqasset
replace augmeqasset = eqasset + unalloceq if host != 983 & source != 93
replace augmeqasset = 0 if host == 983

gen augmdebtasset = debtasset
replace augmdebtasset = debtasset + unallocdebt if host != 983 & source != 93
replace augmdebtasset = 0 if host == 983


// f) For SEFER, we simply prorate unallocad+confidential claims following the allocated claims pattern*/
replace augmeqasset = augmeqasset + shareeqalloc * totunalloceq if source == 93
replace augmdebtasset = augmdebtasset + sharedebtalloc * totunallocdebt if source == 93
...
merge 1:1 year source host using "$temp\Cayman_TIC_Dec.dta", nogenerate
...
drop share_US_KY share_debt_US_KY share_nonus_eq_old share_nonus_eq_new rescale_eq shareeq_KY share_nonus_debt_old share_nonus_debt_new rescale_debt sharedebt_KY
...
sort source host year
	...
	* Fill the gaps for countries that have less than 5 observations
	* group by source and also host 

		* we want to identify them:	
		foreach x in "eq" "debt"{
		* transform missing in 0 
		replace tot`x'asset=0 if missing(tot`x'asset)
		* create a dummy for the source-host that have between 1 and 5 zero values
		bysort source host : gen `x'_zerovalue= sum(tot`x'asset==0)
		bysort source host: egen `x'_max_zerovalue= max(`x'_zerovalue) 
		gen `x'_zerovalue_1_5=1 if `x'_max_zerovalue>=1 & `x'_max_zerovalue<=5
		drop  `x'_max_zerovalue `x'_zerovalue
		} 
		* gen a final missing variable dummy, that is equal to one if at least one of the `x'_zerovalue_1_5 equals 1
		gen zerovalue_1_5=0
		replace zerovalue_1_5=1 if (eq_zerovalue_1_5==1 | debt_zerovalue_1_5==1)

		* Several cases for missing values: 
		//We put n+1 (we only have to put n-1 when the missing values are the last ones).
		bysort source host: egen last_year=max(year)
		sort source host year 
		
		************* first years missing , or missing in the middle *************
		foreach x in "eq" "debt"{	
			forvalue i = 1/5{
				replace help_share_`x' = help_share_`x'[_n+1] if help_share_`x' == 0  & help_share_`x'[_n+1] != 0	& year!=last_year  & zerovalue_1_5==1
			}
		}

		************* last years missing *************
		* identify couples that have last year observation == 0 
		foreach x in "eq" "debt"{	
			bysort source host: gen zero_last_year_`x'=1 if year==last_year & help_share_`x'== 0
			bysort source host: egen zero_last_year_cd_`x'=max(zero_last_year_`x')
		}
		
		* for those couples, replace by the previous value  
		sort source host year 
		foreach x in "eq" "debt"{	
			*forvalue i = 1/5{
			replace help_share_`x' = help_share_`x'[_n-1] if help_share_`x' == 0 & zero_last_year_cd_`x'==1 & help_share_`x'[_n-1] != 0   & zerovalue_1_5==1
			*}
		}

		* impute the 2 remaining variables
		foreach x in "eq" "debt"{	
			replace tot`x'asset = help_share_`x' * help_total_`x' if tot`x'asset == 0 & zerovalue_1_5==1	
		}

		// no replace augmeqasset because already estimated for Cayman
		foreach x in "eq" "debt"{
			replace augm`x'asset = tot`x'asset * share`x'p if augm`x'asset == 0 & zerovalue_1_5==1	& source != 377
		}
	
		drop *zerovalue_1_5* *last_year*
...
/*other countries? -> too long periods missing
	...
	tab sourcename if toteqasset == 0 & host == 111 & help ! =0
	drop help
...
// we assume that Non-US shares given by share of each individual country in 
// total SEFER+SSIO investments in non-US countries

preserve
	use "$temp\TIC_China_Dec.dta", clear
	merge 1:1 year using "$temp\data_IMF_China.dta"
	drop _merge
...
drop *_TIC share
drop reserves_China totaleq_China totaldebt_China
...
preserve
	use "$temp/TIC_update_middleast.dta", clear
	append using "$temp\Bertaut_Judson_middleeast_Dec.dta" // to be consistent with Zucman (2013)
	...
	merge 1:1 year using "$temp\adjust_period.dta"
	drop if _merge == 2 // 2024
	drop _merge
	...
	merge m:1 year using "$temp\shortterm_ratio_FOI.dta"
	assert _merge == 3
	drop _merge
	...
	replace USshare = USshare[_n-1] if USshare == . // critical assumption?
...
// Total and U.S. assets
	...
	drop shareequsp sharedebtusp shareeqpnonus sharedebtpnonus



//----------------------------------------------------------------------------//
// Allocation of missing UK holdings of Irish Fund Shares
//----------------------------------------------------------------------------//
merge 1:1 year source host using "$temp\missing_uk_eqassets.dta", nogen
br year source host eqasset augmeq missing_eqasset_uk if source == 112 & host == 178
replace augmeqasset = augmeqasset + missing_eqasset_uk if source == 112 & host == 178
drop missing_eqasset_uk*
...
// a) by Lane and Milesi-Ferretti (2018) EWNII database 
// b) When aportif_debt is missing, we take 20% of adebt (20% = unweighted mean value of aportif_debt/adebt 
...
// External Wealth of Nations database Jan 2025 (EWN)
merge m:1 source year using "$temp\data_ewn_update.dta"
drop lequity lportif_debt _merge
// ewn = dummy for country in EWN
rename ewn ewn_source
...
// NB: important to subtract IOs (91) which have derived liab, but whose assets are already counted in SEFER 91
// China, Oman, Qatar, Saudi Arabia, UAE, Iran, Iraq
replace toteqasset = augmeqliab if cpis != 1 & source != 924 & source != 449 & source != 453 & source != 456 & source != 429 & source != 433 & source != 466 & source != 91 & aequity == .
replace totdebtasset = augmdebtliab if cpis != 1 & source != 924 & source != 449 & source != 453 & source != 456 & source != 466 & source != 429 & source != 433 & source != 91 & aportif_debt == .
...
// II - Reserve of non-CPIS countries: we use IFS data and assume 75% invested in securities
// (74% in debt and 1% in equity) 
preserve
	use "$temp\data_foreignexchange_update.dta", clear
...
// 1) EWNII
preserve
	use "$temp\data_ewn_update.dta", clear
	rename source host
	sort host year
	keep year host lequity lportif_debt ewn
	tempfile ewn
	save `ewn'
restore

sort host year
merge m:1 host year using `ewn'
rename ewn ewn_host
...
// 1bis) Correction for EWNII data
	...
	// correction for EWN reported liab < raw CPIS derived liab */
	sort host year
...
// 2) Add liabilities for international organizations
	...
	merge m:1 year host using "$temp\BIS_total_debt_IO.dta"
	drop if _merge==2
	drop _merge
	/* br year source host eqasset debtasset debtliab_host eqliab_host total_debt_BIS if host == 91 */
	replace eqliab_host = rawderivedeq if host == 91
	replace debtliab_host = total_debt_BIS if host == 91
	drop total_debt_BIS

// 3) Augmented derived liabilities for non EWN nations 
	sort host year
	by host year: egen derivedeqliab_host = total(augmeqasset) 
	by host year: egen deriveddebtliab_host = total(augmdebtasset)
	replace eqliab_host = derivedeqliab_host if eqliab_host == . & host != 983 & host != 91 & host != 93
	replace debtliab_host = deriveddebtliab_host if debtliab_host == . & host != 983 & host != 91 & host != 93
...
drop if year > 2023
save "$temp\data_full_matrices.dta", replace
...
save "$temp\gap_source_update.dta", replace

// Produce global portfolio asset liability gap as input for Part II
collapse (sum) gapeq_source gapdebt_source, by(year) 
gen gapport_total = gapeq_source + gapdebt_source
keep year gapport_total


	save "$work/global_portfolio_gap.dta", replace
//----------------------------------------------------------------------------//
